/**
 * Tools config cho Gemini Live API — MTPC Chatbot
 * 
 * Các tool được hỗ trợ:
 * - search_mtpc_knowledge: Tìm kiếm trong cơ sở dữ liệu kiến thức của MTPC
 * - navigate_to_page: Chuyển hướng người dùng đến một trang của MTPC
 */
import { getRAG } from "@/lib/chatbot/singleton";
      ...
      functionDeclarations: [
        ...
        },
        {
          ...
          description:
            ...
            "Không gọi tool này mà không có sự đồng ý của người dùng.",
          parameters: {
            type: "OBJECT",
            properties: {
              url: {
  ...
  return {
    search_mtpc_knowledge: async (args) => {
      ...
      try {
        ...
        const result = await rag.search(query.trim());
    ...
    navigate_to_page: async (args) => {
      ...
      // Tool này trả về lệnh điều hướng — client sẽ xử lý
